...
export class Router {
    ...
    add(method: string, path: string, handler: Function) {
        ...
        for (; i < l ; ++i) {
            ...
            if (ch === Kind.Colon) {
                ...
                if (i === l) {
                    this.insert(method, path.substring(0, i), Kind.Parameter, pnames, handler);
                    return;
                }
    ...
    insert(method: string, path: string, kind: Kind, pnames?: any, handler?: Function) {
        ...
        while (true) {
            ...
            if (l < pl) {
                // split node
                n = new Node(prefix.substring(l), current.children, current.kind, current.map);
                // add to parent
                current.children = [n];
    ...
    find(method: string, path: string, cn?: Node, n = 0, result = [undefined, []]) {
        cn = cn || this.tree;
